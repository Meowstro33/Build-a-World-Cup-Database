...
# select t1.name as name from games g join teams t1 on g.winner_id = t1.team_id where g.year = 2014 and g.round = 'Eighth-Final' UNION select t2.name as name from games g join teams t2 on g.opponent_id = t2.team_id  where g.year = 2014 and g.round = 'Eighth-Final';
